...
# ==========================================================
# ACTIVE PREDICTIONS ENGINE
# ==========================================================

ACTIVE_PREDICTION_SWITCH_HOUR = 17      # 5 PM Sydney


def get_active_prediction_target_date():
    """
    Before 5PM:
    ...
    now = get_current_aest()

    if now.hour >= ACTIVE_PREDICTION_SWITCH_HOUR:
        return now.date() + timedelta(days=1)

    return now.date()


def prediction_lock_status(kickoff):
...
def prediction_lock_description(kickoff):
    ...
    return "Predictions still editable"


def prediction_is_locked(kickoff):

    return get_current_aest() >= kickoff


def get_target_day_matches(group_df, knockout_df):
    """
    Returns ONLY matches for the target day.

    Target day =
        Today (before 5PM)
        Tomorrow (5PM onwards)

    Completed matches are excluded.
    """

    target_day = get_active_prediction_target_date()
    ...
    if not group_df.empty:
        ...
        tmp = tmp[
            ...
            &
            (tmp["Kickoff_Date"] == target_day)
        ]

        tmp["Competition"] = "Group"

        frames.append(tmp)

    if not knockout_df.empty:

        tmp = knockout_df.copy()
    ...
    combined = combined.sort_values("Kickoff_AEST")

    return combined


def get_prediction_value(row, participant, field):

    column = f"{participant}_{field}"
    ...
    return clean_country_name(value)


def build_active_prediction_table(match_row):
    """
